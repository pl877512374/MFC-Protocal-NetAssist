// ProtocolAnalysis.cpp : 定义 DLL 应用程序的导出函数。
//DLL_API
...
bool f_AddAng = false;//是否需要角度补偿标志位
...
HWND m_hReciWnd;
SOCKET brocastSocket;//udp广播
SOCKADDR_IN sin_server;
bool f_BroThreadCon = false;
char *sendbroBuf = NULL;//广播指令
int nbroLen = 0;//广播指令长度
...
bool ConnectServer(HWND hReciveWnd, NetConnectPara*NetStruct)
{
	...
	// 创建客户端套接字
	sockClient = socket(AF_INET, SOCK_STREAM, 0);//设置非阻塞模式
...
void OnNetRecv(char *pDataBuf, int nDataBufSize)
{
	...
	if (g_cNetRecvBuf[g_n32NetRecvID][0] == (char)0xFF &&
		...
		g_cNetRecvBuf[g_n32NetRecvID][13] == (char)0x07 && g_cNetRecvBuf[g_n32NetRecvID][11] == 0x02)   //导航激光器
	{
...
#pragma endregion
	...
	else if (g_cNetRecvBuf[g_n32NetRecvID][0] == (char)0x02 && g_cNetRecvBuf[g_n32NetRecvID][1] != (char)0x02)   //导航激光器 sick协议 判断单个02包帧头
	{
